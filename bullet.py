...
# Code to check if two segments intersects,
...
class Bullet:
    ...
    def intersectsWithCircle(self, circle_pos_x, circle_pos_y, radius):
        ...
        # Implementation of algorithm found on
        ...
        proj_distance_len = distance.x * move_unit.x + distance.y * move_unit.y

        proj_distance = Coord(proj_distance_len * move_unit.x,
                              proj_distance_len * move_unit.y)

        if(proj_distance_len < 0):
            closest = Coord(self.pos.x, self.pos.y)
        elif(proj_distance_len > move_normal):
            closest = Coord(self.pos.x + self.move.x,
                            self.pos.y + self.move.y)
        else:
            closest = Coord(self.pos.x + proj_distance.x,
                            self.pos.y + proj_distance.y)

        closest_distance = Coord(circle_pos.x - closest.x,
                                 circle_pos.y - closest.y)

        closest_distance_length = math.sqrt(closest_distance.x ** 2
                                            + closest_distance.y ** 2)
    ...
    def intersectsWithRectangle(self,
                                rectangle_pos_x,
                                rectangle_pos_y,
                                rectangle_length_x,
                                rectangle_length_y):
        ...
        # Here we check if the bullet is not entirely inside of the square
        if(rectangle_pos.x < self.pos.x) \
            and (rectangle_pos.y
                 < self.pos.y) \
            and (rectangle_pos.x + rectangle_length.x
                 > self.pos.x + self.move.x) \
            and(rectangle_pos.y + rectangle_length.y
                > self.pos.y + self.move.y):
            return True
        # ABCD are the coordinates of the edges of the rectangle
        rectangle_A = Coord(rectangle_pos.x,
                            rectangle_pos.y)
        rectangle_B = Coord(rectangle_pos.x,
                            rectangle_pos.y + rectangle_length.y)
        rectangle_C = Coord(rectangle_pos.x + rectangle_length.x,
                            rectangle_pos.y)
        rectangle_D = Coord(rectangle_pos.x + rectangle_length.x,
                            rectangle_pos.y + rectangle_length.y)
        ...
        # then the bullet and the rectangle doesn't intersects
